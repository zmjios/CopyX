...
class HotKeyManager: NSObject, ObservableObject {
    ...
    private func createHistoryWindow() {
        // 1. 获取当前鼠标所在的屏幕，如果失败则回退到主屏幕
        let screen = getScreenWithMouse() ?? NSScreen.main
        guard let screen = screen else {
            NSLog("无法获取屏幕信息")
            return
        }
        
        // 2. 基于正确的屏幕计算窗口位置和动画起始位置
        let currentDisplayMode = WindowDisplayMode(rawValue: displayMode) ?? .bottom
        ...
        // 创建内容视图
        let contentView = ClipboardHistoryView(onClose: self.hideClipboardHistory)
        ...
        // 执行入场动画
        animateWindowAppearance(window: window, finalFrame: windowRect, mode: currentDisplayMode)

        // 设置点击外部关闭窗口的监听
        setupClickOutsideMonitor(for: window)
    }
    ...
    private func animateWindowAppearance(window: NSWindow, finalFrame: NSRect, mode: WindowDisplayMode) {
        switch mode {
        case .bottom:
            // 底部模式：从下方滑入
            NSAnimationContext.runAnimationGroup({ context in
    ...
    private func setupClickOutsideMonitor(for window: NSWindow) {
